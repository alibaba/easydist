...
from torch.fx.experimental.symbolic_shapes import (ShapeEnv, SymDispatchMode, SymNode)
...
__all__ = [
    "PythonKeyTracer", "dispatch_trace", "my_make_fx", "DecompositionInterpreter", "py_sym_types",
    ...
    if isinstance(val, FakeTensor):
        ...
        if not val.is_sparse:
            ...
            with fake_tensor_mode:
                return torch.empty_strided(val.shape,
                                           val.stride(),
                                           device=val.device,
                                           dtype=val.dtype)
        else:
...
def track_tensor(tensor, proxy, *, constant, tracer):
    ...
    try_set_proxy_slot(tensor.storage_offset(),
...
def proxy_call(proxy_mode, func, pre_dispatch, args, kwargs):
    ...
    f_args, f_kwargs = pytree.tree_map_only(torch.Tensor, fetch_tensor_proxy(tracer),
                                            (args, kwargs))
    ...
    if torch.Tag.data_dependent_output in func.tags:
        ...
        if all_constant:
            const_args, const_kwargs = pytree.tree_map_only(_ProxyTensor, lambda t: t.constant,
                                                            (f_args, f_kwargs))
        ...
        if pytree.tree_all_only(torch.Tensor, lambda t: not isinstance(t, FakeTensor),
                                (args, kwargs)):
            raise RuntimeError(
                ...
                "in your my_make_fx call.")
    proxy_args, proxy_kwargs = pytree.tree_map_only(
        (SymInt, SymFloat, SymBool), fetch_sym_proxy(proxy_mode.tracer),
    ...
    proxy_out = proxy_mode.tracer.create_proxy('call_function',
                                               func,
                                               proxy_args,
                                               proxy_kwargs,
                                               name=proxy_mode.tracer.graph._target_to_str(
                                                   func.overloadpacket.__name__))
    ...
    any_constant = pytree.tree_any_only(_ProxyTensor, lambda t: t.constant is not None,
                                        (f_args, f_kwargs))
    ...
    elif (torch.Tag.nondeterministic_seeded not in func.tags
...
def wrap_key(f, tensors, tracer, pre_dispatch: bool):
    ...
    def wrapped(*proxies):
        ...
        out = pytree.tree_map_only(torch.Tensor,
                                   lambda t: get_proxy_slot(t, tracer, t, lambda x: x.proxy), out)
        out = pytree.tree_map_only((SymInt, SymFloat, SymBool),
                                   lambda t: get_proxy_slot(t.node, tracer)(), out)
        return out
...
# my_make_fx through a regular Interpreter
...
def wrapper_and_args_for_my_make_fx(func, args, kwargs):
    # my_make_fx doesn't support kwargs, so we need to do this flattening
...
def my_make_fx(f,
    ...
    def wrapped(*args):
        ...
        try:
            ...
            def wrap_fake(x):
                ...
                if isinstance(x, torch.Tensor):
                    ...
                    return fake_tensor_mode.from_tensor(
                        x, source=source)  # type: ignore[attr-defined]
            ...
            # purpose of `my_make_fx` is to produce graphmodules as a side effect; its internal execution is
...
def get_isolated_graphmodule(func, args, kwargs, tracing_mode="real"):
    ...
    """
    wrapped, all_args = wrapper_and_args_for_my_make_fx(func, args, kwargs)

    with disable_proxy_modes_tracing():
        gm = my_make_fx(wrapped, tracing_mode=tracing_mode)(all_args)
    return gm

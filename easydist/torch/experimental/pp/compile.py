...
from easydist.torch.compiler import preprocess_traced_graph, easydist_shard, sol_rdy_cond, fetch_strategy, sharded_tensor, dtensor_to_tensor
...
def _compile(func, tracing_mode, init_helper, input_signature, args, kwargs):
    flatten_args_kwargs, args_kwargs_specs = pytree.tree_flatten([args, kwargs])

    module, opt = None, None
    for i, arg in enumerate(flatten_args_kwargs):
        if isinstance(arg, torch.nn.Module):
            assert module is None, "Only support single nn.Module in args now"
            module = _from_tracing(arg)
            flatten_args_kwargs[i] = module

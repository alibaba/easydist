...
def train_submodule(train_step, name, module, *prev_mod_input):
    ...
    warp_fn = lambda item: [item] if isinstance(item, torch.Tensor) else item
...
def train_example():
    ...
    input = train_submodule(train_step_submod0, "submod_0", pipe.split_gm.submod_0, *input)

    input = [t.detach() for t in input]
    train_submodule(train_step_submod1, "submod_1", pipe.split_gm.submod_1, *input)

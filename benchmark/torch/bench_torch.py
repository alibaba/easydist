...
from metadist.torch import (enable_transform, get_input_strategy, compile, set_device_mesh,
                            shard_module)
...
def bench_metadist(model, data_in):
    ...
    timer = MDTimer(train_step_partial, in_ms=False)
...
def bench_metadist_old(model, data_in):
    world_size = torch.distributed.get_world_size()

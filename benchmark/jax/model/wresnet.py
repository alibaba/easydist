...
class BottleneckResNetBlock(nn.Module):
    ...
    def __call__(self, x):
        residual = x
        y = self.conv(self.filters * self.width_factor, (1, 1))(x)
        y = self.norm()(y)
        y = self.act(y)
...
def resnet50():
    return ResNet(stage_sizes=[3, 4, 6, 3], block_cls=BottleneckResNetBlock)


def wresnet50():
    return ResNet(block_cls=BottleneckResNetBlock, stage_sizes=[3, 4, 6, 3], width_factor=2)


def wresnet101():
    return ResNet(block_cls=BottleneckResNetBlock, stage_sizes=[3, 4, 23, 3], width_factor=2)

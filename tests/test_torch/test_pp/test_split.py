...
from torchvision.models import (alexnet, densenet121, efficientnet_b0,
                                resnet18, swin_t, vgg19, vit_b_16)

from easydist import easydist_setup
import easydist.config as mdconfig
from easydist.torch.compile_auto import (easydist_shard,
                                         preprocess_traced_graph,
                                         sharded_tensor)
from easydist.torch.decomp_utils import EASYDIST_DECOMP_TABLE
from easydist.torch.device_mesh import (get_device_mesh, get_pp_rank,
                                        get_pp_size, set_device_mesh,
                                        spmd_device_mesh)
...
from easydist.torch.experimental.pp.split_utils import (
    clear_pp_compile_states, get_updated_params_states)
from easydist.torch.experimental.pp.utils import save_graphviz_dot
from easydist.torch.init_helper import (SetParaInitHelper, init_contiguous_buf,
                                        materialize_zero)
...
def train_step_gpt(input, label, model, opt):
    ...
    loss = 0
    for key in [
            'attentions', 'cross_attentions', 'hidden_states', 'pooler_output',
            'pask_key_values', 'last_hidden_state'
    ]:
        if hasattr(out, key) and (attr := getattr(out, key)) is not None:
            if isinstance(attr, torch.Tensor):
                attr_broadcast = attr.permute(
                    [i for i in range(attr.dim() - 1, -1, -1)])
                loss += (attr_broadcast - label).pow(2).mean()
            elif isinstance(attr, (tuple, list)):
                for a in attr:
                    attr_broadcast = a.permute(
                        [i for i in range(a.dim() - 1, -1, -1)])
                    loss += (attr_broadcast - label).pow(2).mean()
    if opt is not None:
        loss.backward()
        opt.step()
...
def test_main(module, split_ann_or_policy, rand_input_gen_method,
              train_step_func):
    ...
    label = torch.tensor([random.random()
                          for _ in range(rand_input.shape[0])]).to(device)
    ...
    for _ in range(epochs):
        ...
        label = torch.tensor(
            [random.random() for _ in range(rand_input.shape[0])]).to(device)
    ...
    with torch.no_grad():
        for rand_input, label in dataset:
            ...
            input_nodes = {
                node_name: sharded for node_name, sharded in zip(
    ...
    assert all(
        torch.allclose(state_dict[k], state_dict_copy[k]) for k in state_dict_copy)
    assert all(
...
def _compile_auto_local(func,
    ...
    def stateless_func(func, params, buffers, named_states, args, kwargs):
        ...
        with stateless._reparametrize_module(
                ...
                tie_weights=True
        ) if module else nullcontext(), _rematerialize_optimizer(
    ...
    args_split, kwargs_split = split_args_kwargs_into_chunks(
        args, kwargs, num_chunks, args_chunk_spec, kwargs_chunk_spec)

    with _enable_compile(), SplitPatcher(
            module, opt) if schedule_cls else nullcontext():
    ...
    if rank == 0:
        shape_info, opt_strategy, sharding_strategy, args_strategy, state_io_map = easydist_shard(
            traced_graph, state_tensor_num, input_signature, params, buffers,
            named_states, args, kwargs)

        with sol_rdy_cond:
            global sharding_sol
            sharding_sol = [
                shape_info, opt_strategy, sharding_strategy, args_strategy,
                state_io_map
            ]
    ...
    with spmd_device_mesh():
        if mdconfig.use_dtensor:
            sharded_graph = sharding_transform_dtensor(traced_graph,
                                                       sharding_strategy)
        else:
            sharded_graph = sharding_transform(traced_graph, opt_strategy,
                                               state_io_map)
    ...
    if not mdconfig.use_dtensor:
        ...
        if mdconfig.override_dtensor_rule is True:
            sharded_graph = rule_override_by_graph(sharded_graph, opt_strategy,
                                                   shape_info)
    ...
    def shard_states(params, buffers, named_states):
        with spmd_device_mesh():
            ...
            buffers_strategy = args_strategy[len(params):len(params) +
                                            len(buffers)]
            ...
            for idx, param_name in enumerate(params):
                ...
                if mdconfig.use_contiguous_buffer:
                    contiguous_buf[index:index +
                                    size] = params[param_name]._local_tensor.view(
                                        -1)
            ...
            for idx, buffer_name in enumerate(buffers):
                ...
                materialize_fn = partial(materialize_fn,
                                            ...
                                            materialization_device=device)
                buffers[buffer_name] = sharded_tensor(
                        buffers[buffer_name],
            ...
            flat_named_states, named_states_spec = pytree.tree_flatten(
                    named_states)
            state_tensor_num = len(params) + len(buffers)
            materialize_fn = partial(materialize_zero,
                                        materialization_device=device)
            ...
            named_states = pytree.tree_unflatten(flat_named_states,
                                                    named_states_spec)
        ...
        for name in dict(module.named_parameters()):
            with torch.no_grad():
                rsetattr(module, name + ".grad",
                         torch.zeros_like(rgetattr(module, name).data))
...
if __name__ == '__main__':
    ...
    test_main(alexnet(), split_into_equal_size(3), gen_rand_input_imagenet,
              train_step)
    test_main(densenet121(), split_into_equal_size(5), gen_rand_input_imagenet,
              train_step)
    test_main(efficientnet_b0(), split_into_equal_size(10),
              gen_rand_input_imagenet, train_step)
    test_main(resnet18(), split_into_equal_size(4), gen_rand_input_imagenet,
              train_step)
    test_main(swin_t(), split_into_equal_size(10), gen_rand_input_imagenet,
              train_step)
    test_main(vgg19(), split_into_equal_size(3), gen_rand_input_imagenet,
              train_step)
